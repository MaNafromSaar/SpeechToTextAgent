// --- Standalone Device Enumeration for index.html (non-intrusive) ---
async function setupAudioDevicesStandalone() {
    try {
...
document.addEventListener('DOMContentLoaded', () => {
    setupAudioDevicesStandalone();
});
// --- Standalone Device Enumeration for index.html ---
async function setupAudioDevicesStandalone() {
    try {
        const devices = await navigator.mediaDevices.enumerateDevices();
        const audioInputs = devices.filter(device => device.kind === 'audioinput');
...
// --- Device Enumeration Logic (standalone, like testing.html) ---
...
document.addEventListener('DOMContentLoaded', () => {
    setupAudioDevices();
    // ...existing code for other event listeners and initialization...
});
...
// ...existing code...
    ...
    async setupAudioDevices() {
        try {
            ...
            // Enumerate devices first (no permissions required)
            let devices = await navigator.mediaDevices.enumerateDevices();
            let audioInputs = devices.filter(device => device.kind === 'audioinput');

            // If all labels are empty, request permission and re-enumerate
            const allLabelsEmpty = audioInputs.every(device => !device.label);
            if (allLabelsEmpty) {
                try {
            ...
            const currentSelection = audioInputSelect.value;
            audioInputSelect.innerHTML = '<option value="">Select Input Device...</option>';

            if (audioInputs.length === 0) {
                audioInputSelect.innerHTML = '<option value="">No audio inputs found</option>';
                this.showError('No audio input devices detected');
            ...
            audioInputs.forEach((device, idx) => {
                const option = document.createElement('option');
                option.value = device.deviceId;
                // Always show deviceId if label is missing, for clarity
                if (device.label && device.label.trim() !== "") {
                    option.textContent = device.label;
                } else {
                    option.textContent = `Microphone ${idx + 1} (${device.deviceId})`;
                }
                audioInputSelect.appendChild(option);
            });

            if (currentSelection) {
                const optionExists = Array.from(audioInputSelect.options).some(
                    opt => opt.value === currentSelection
                );
                if (optionExists) {
                    audioInputSelect.value = currentSelection;
                }
            }
...
showDeviceLoading(show) {
    ...
    if (show) {
        ...
        refreshBtn.innerHTML = '<i class="fas fa-spinner fa-spin mr-1"></i>';
    } else {
...
showError(message) {
    ...
    errorDiv.className = 'bg-red-900 text-red-100 p-3 rounded-lg mb-4 flex items-center';
    errorDiv.innerHTML = `
    ...
    if (panel) {
        ...
        this.hideAudioError();
        panel.insertBefore(errorDiv, panel.querySelector('h2').nextSibling);
    } else {
